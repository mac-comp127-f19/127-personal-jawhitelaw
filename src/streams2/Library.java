...
public class Library {
    ...
    public List<String> findTitlesByAuthor(String authorName) {
        ...
        return books.stream().filter(a -> a.getAuthor().equals(authorName)).map(Book :: getTitle).collect(toList());
    ...
    public List<String> findTitlesAndAuthorsInGenre(String genre) {
        ...
        return books.stream().filter(a -> a.getGenres().contains(genre)).map(b -> b.getTitle() + " by " + b.getAuthor()).collect(toList());
    ...
    public long countBooksInGenre(String genre) {
        ...
        return books.stream().filter(d -> d.getGenres().contains(genre)).collect(toList()).size();
    }
    ...
    public List<Book> findShortestBooks(int n) {
        ...
        ArrayList<Book> tempBooks = new ArrayList<>(books);
        tempBooks.sort(Comparator.comparing(Book::getPageCount));
        List<Book> result = new ArrayList<Book>();

        for (int i=0; i<Math.min(n,tempBooks.size()); i++){
            result.add(tempBooks.get(i));
        }
    ...
    public List<Book> findBooksInGenreSortedByLength(String genre) {
        ...
        return books.stream().filter(d -> d.getGenres().contains(genre)).sorted(Comparator.comparing(Book::getPageCount)).collect(toList());
    }
